...
// tenant name
const tenantName = 'cypress';

// implementation names
const implementationNames = ['uno-widgets-do-not-touch'];
...
(async () => {
  ...
  const implementations = await Promise.all(
    implementationNames.map((implementationName) =>
      loadImplementation(environment, `https://${tenantName}.humany.net/${implementationName}`)
    )
  );
  ...
  implementations.forEach((implementation) => {
    ...
    bootstrap(implementation, (config) => {
      ...
      const widget = implementation.widgets.get('uno-upgrade-portal');
      widget.activate();
